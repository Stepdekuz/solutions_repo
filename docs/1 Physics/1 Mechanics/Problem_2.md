...
theta0 = np.radians(20)  # initial angle (converted to radians)
omega0 = 0               # initial angular velocity
t_span = (0, 30)
t_eval = np.linspace(*t_span, 2000)

# Pendulum differential equation
def pendulum(t, y, b=0, A=0, omega_drive=0):
    theta, omega = y
    dtheta_dt = omega
...
def simulate_and_plot(title, color, b=0, A=0, omega_drive=0):
    sol = solve_ivp(pendulum, t_span, [theta0, omega0], t_eval=t_eval, args=(b, A, omega_drive))
    theta, omega = sol.y
    ...
    # Time series plot
    axs[0].plot(sol.t, theta, color=color)
    axs[0].set_title("Time Series")
    axs[0].set_xlabel("Time (s)")
    axs[0].set_ylabel("Angle (rad)")

    # Phase space plot
    axs[1].plot(theta, omega, color=color)
...
# 1) Simple Pendulum (no damping, no driving force)
simulate_and_plot("1) Simple Pendulum", color="orangered", b=0, A=0)

# 2) Damped Pendulum (damping, no force)
simulate_and_plot("2) Damped Pendulum", color="slateblue", b=0.5, A=0)

# 3) Forced Pendulum (force only, no damping)
simulate_and_plot("3) Forced Pendulum", color="teal", b=0, A=1.2, omega_drive=2.0)

# 4) Forced Damped Pendulum (resonant behavior)
simulate_and_plot("4) Forced Damped Pendulum (Resonance)", color="mediumseagreen", b=0.5, A=1.2, omega_drive=2.0)

# 5) Forced Damped Pendulum (chaotic behavior)
simulate_and_plot("5) Forced Damped Pendulum (Chaos)", color="tomato", b=0.5, A=1.2, omega_drive=0.666)

```
link:[colab](https://colab.research.google.com/drive/1mwJx9KSNeh55Mf6e7mD_ShcQs9oz3kVb?usp=sharing)