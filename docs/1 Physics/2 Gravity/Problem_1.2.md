...
v1 = [7.9, 3.6, 42.1]     # Orbital velocities (km/s)
v2 = [11.2, 5.0, 59.5]    # Escape velocities (km/s)
v3 = [16.7, 9.0, 71.0]    # Solar escape (approximate, km/s)

x = np.arange(len(planets))  # label locations
width = 0.25

# Plot
fig, ax = plt.subplots(figsize=(10, 6))
rects1 = ax.bar(x - width, v1, width, label='First Cosmic Velocity', color='cornflowerblue')
rects2 = ax.bar(x, v2, width, label='Second Cosmic Velocity', color='darkorange')
rects3 = ax.bar(x + width, v3, width, label='Third Cosmic Velocity', color='limegreen')

# Labels and title
ax.set_ylabel('Velocity (km/s)', fontsize=12)
ax.set_title('Cosmic Velocities for Earth, Mars, and Jupiter', fontsize=14)
ax.set_xticks(x)
