...
# Dünya yarıçapı (yaklaşık)
R = 6.371e6  # metre

# Dünya'nın dairesi (daha yumuşak çizim için daha fazla nokta)
theta_earth = np.linspace(0, 2 * np.pi, 1000)
earth_x = R * np.cos(theta_earth)
earth_y = R * np.sin(theta_earth)

# Basit yörünge verisi
theta_traj = np.linspace(0, np.pi / 2, 300)
trajectory_x = R * np.cos(theta_traj)
trajectory_y = R * np.sin(theta_traj) + 0.2 * R  # Hafif yukarı çekilmiş

# Grafik
plt.figure(figsize=(7, 7))
plt.plot(trajectory_x, trajectory_y, color='darkorange', linewidth=2, label='Trajectory')
plt.fill(earth_x, earth_y, color='mediumseagreen', alpha=0.8, label='Earth')
...
for v0, color in zip(velocities, colors):
    initial_state = [r0[0], r0[1], 0, v0]
    sol = solve_ivp(gravity, t_span, initial_state, t_eval=t_eval, rtol=1e-8)

    x = sol.y[0] / 1000  # to km
    y = sol.y[1] / 1000
    ax.plot(x, y, color=color, label=f'v = {v0} m/s', linewidth=2)

# Draw Earth
earth = plt.Circle((0, 0), R_earth / 1000, color='cadetblue', alpha=0.4)
ax.add_patch(earth)

# Styling
ax.set_title("Case 4: Near-Escape Velocities", fontsize=13)
ax.set_xlabel("x (km)")
ax.set_ylabel("y (km)")
ax.grid(True)
ax.set_aspect('equal')
